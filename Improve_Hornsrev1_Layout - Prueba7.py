...
for s in range(500):
    ...
    y_sol = []
    for i in range(0,(WT_Num-1)):
        x_sol.append(rnd.uniform(posit_x_min,posit_x_max))
        y_sol.append(rnd.uniform(posit_y_min,posit_y_max))
            ...
            for s in bestsolutions:
                for i in range(0,(WT_Num-1)):
                    ...
                    while good_new_gen_x == False:
                        new_gen_x = (s[1][0][i]*rnd.uniform(0.98,1.02))
                        if ((new_gen_x <= posit_x_max) and (new_gen_x >= posit_x_min)):
                            good_new_gen_x = True
                    while good_new_gen_y == False:
                        new_gen_y = (s[1][1][i]*rnd.uniform(0.98,1.02))
                        if ((new_gen_y <= posit_y_max) and (new_gen_y >= posit_y_min)):
                            good_new_gen_y = True
                    x_element_i.append(new_gen_x)
                    y_element_i.append(new_gen_y)
