...
def list_randon_values(lst1,lst2,border):
    lst1_rnd = []
    lst1_rnd_val = 0
    lst2_rnd = []
    lst2_rnd_val = 0
    ...
    for s in lst:
        ...
        while good_lst_rnd_val == False:
            lst1_rnd_val = sx + (Iter_Length * rnd.uniform(-1,1))
            lst2_rnd_val = sy + (Iter_Length * rnd.uniform(-1,1)) 
...
def getSolutions():
    ...
    for x in iter_lengths:
        ...
        for s in range(Iter_Num):
            ...
            y_rnd = []
            xy_ref = ( (wt_x,wt_y) )
            x_sol,y_sol = list_randon_values(wt_x,wt_y,mesh_border)
...
def getGenSolutions(bestsolution):
    NewGen_i_best = []
    aep_best_sol_i = []
    Delta_aep_best_sol_i = []
    solutions_dic_var_iter_lenght = {"0":[],"1":[], "2":[]}
        ...
        for x in iter_lengths:
            ...
            for _ in range(Iter_Num):
                ...
                for s in bestsolution:
                    ...
                    new_gen_x_rnd = []
                    new_gen_y_rnd = []
                    new_gen_x_rnd,new_gen_y_rnd = list_randon_values(new_gen_x,new_gen_y,mesh_border)
                x_sol = new_gen_x_rnd
                y_sol = new_gen_y_rnd                
...
global bestsolution
bestsolution = []
iter_lengths = {"0": Iter_Length,"1": Iter_Length-5,"2": Iter_Length+5}
bestsolution = getSolutions()

# Getting the succesive Genetic Generations
SolGen = getGenSolutions(bestsolution)
