...
def select_best_fitness_by_iteration(df,order_column,max_column):
    # Encontrar los índices de las filas con el valor máximo para cada grupo
    indices_maximos = df.groupby(order_column)[max_column].idxmax()
    # Seleccionar las filas correspondientes a los índices máximos
    df_maximos = df.loc[indices_maximos]
    return df_maximos
...
nombres_columnas_sol = ['Iteration Length','Iteration','xy', 'Fitness Function_xy_sol']
# Crea el DataFrame
...
nombres_columnas_gensol = ['Iteration','Iteration Length','Generation', 'xy', 'Fitness Function_xy_gensol']
# Crea el DataFrame
